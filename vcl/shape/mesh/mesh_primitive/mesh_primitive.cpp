...
mesh mesh_primitive_ellipsoid(float a, float b, float c, const vec3& p0, size_t Nu, size_t Nv)
{
    ...
    for( size_t ku=0; ku<Nu; ++ku ) {
        for( size_t kv=0; kv<Nv; ++kv ) {
            ...
            const vec3 p  = {x,y,z};      // Position (centered)
            const vec3 n_vec = {(2*x)/(a*a),(2*y)/(b*b),(2*z)/(c*c)};
            const vec3 n  = normalize(n_vec); // Normal
            const vec2 uv = {v,u};        // Texture-coordinates
...
mesh mesh_primitive_filled_cone(float radius,  const vec3& p1, const vec3& p2, size_t Nu, size_t Nv)
{
    const vec3 p12 = p2-p1;
    const float L = norm(p12);
    const vec3 dir = p12/L;
    ...
    for(size_t ku=0; ku<Nu; ++ku)
    {
        ...
        const vec3 n = R * vec3(0, 0, -1);

        shape.position.push_back( p );
        shape.normal.push_back( n );
    }

    shape.position.push_back(p1);
    shape.normal.push_back( R * vec3(0, 0, -1) );

    // Connectivity bottom
    for(size_t k=0; k<Nu; ++k)
    {
        const unsigned int u0 = Nu*Nv + k;
        const unsigned int u1 = (k+1)%(Nu) + (Nu*Nv);
        const unsigned int u2 = Nu*Nv+Nu;

        const uint3 t2 = {u0, u1, u2};

        shape.connectivity.push_back(t2);
    }
...
mesh mesh_primitive_frame(float sphere_radius, float cylinder_radius, float cone_radius, float cone_length)
{
    ...
    mesh cylinder_x = mesh_primitive_cylinder(cylinder_radius,{0,0,0},{1.0f-cone_length,0,0}); cylinder_x.fill_color_uniform({.9f,0,0});
    mesh cylinder_y = mesh_primitive_cylinder(cylinder_radius,{0,0,0},{0,1.0f-cone_length,0}); cylinder_y.fill_color_uniform({0,.9f,0});
    mesh cylinder_z = mesh_primitive_cylinder(cylinder_radius,{0,0,0},{0,0,1.0f-cone_length}); cylinder_z.fill_color_uniform({0,0,.9f});

    mesh cone_x = mesh_primitive_filled_cone(cone_radius,{1.0f-cone_length,0,0},{1.0f,0,0});  cone_x.fill_color_uniform({.9f,0,0});
    mesh cone_y = mesh_primitive_filled_cone(cone_radius,{0,1.0f-cone_length,0},{0,1.0f,0});  cone_y.fill_color_uniform({0,.9f,0});
    mesh cone_z = mesh_primitive_filled_cone(cone_radius,{0,0,1.0f-cone_length},{0,0,1.0f});  cone_z.fill_color_uniform({0,0,9.f});

    mesh quad_x = mesh_primitive_quad({0,0,0},{0,0.25,0},{0,0.25,0.25},{0,0,0.25});  quad_x.fill_color_uniform({.9f,0,0});
    mesh quad_y = mesh_primitive_quad({0,0,0},{0.25,0,0},{0.25,0,0.25},{0,0,0.25});  quad_y.fill_color_uniform({0,.9f,0});
    mesh quad_z = mesh_primitive_quad({0,0,0},{0.25,0,0},{0.25,0.25,0},{0,0.25,0});  quad_z.fill_color_uniform({0,0,.9f});
