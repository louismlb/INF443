...
void load_shaders(std::map<std::string,GLuint>& shaders)
{
    ...
    shaders["raytracing"] = create_shader_program("scenes/shared_assets/shaders/raytracing_quad/raytrace.vert.glsl","scenes/shared_assets/shaders/raytracing_quad/raytrace.frag.glsl");
    ...
    shaders["mesh_fixed"] = create_shader_program("scenes/shared_assets/shaders/mesh_fixed/fixed.vert.glsl","scenes/shared_assets/shaders/mesh_fixed/fixed.frag.glsl");
    shaders["mesh_bright"] = create_shader_program("scenes/shared_assets/shaders/mesh_bright/bright.vert.glsl","scenes/shared_assets/shaders/mesh_bright/bright.frag.glsl");
    ...
    shaders["sea"] = create_shader_program("scenes/shared_assets/shaders/sea/shader.vert.glsl", "scenes/shared_assets/shaders/sea/shader.frag.glsl");
    shaders["sea_wireframe"] = create_shader_program("scenes/shared_assets/shaders/sea_wireframe/shader.vert.glsl","scenes/shared_assets/shaders/sea_wireframe/shader.geom.glsl","scenes/shared_assets/shaders/sea_wireframe/shader.frag.glsl");
...
{
    scene.frame_camera = mesh_drawable(mesh_primitive_frame(0.03f, 0.015f, 0.08f, 0.25f)); // (0.15f, 0.05f, 0.15f, 0.3f)
    scene.frame_camera.uniform.transform.scaling = 0.3f; // 0.2f
    scene.frame_camera.shader = shaders.at("mesh"); // "mesh_bright" for unified color

    scene.frame_worldspace = mesh_drawable(mesh_primitive_frame(0.03f, 0.015f, 0.08f, 0.25f)); // (0.05f, 0.015f, 0.05f, 0.1f)
    scene.frame_worldspace.shader = shaders.at("mesh"); // "mesh_bright" for unified color

    int width=0, height=0;
    glfwGetWindowSize(gui.window, &width, &height);
    const float aspect_ratio = width/static_cast<float>(height);

    scene.camera.perspective = perspective_structure( 40*3.14f/180, aspect_ratio, 0.01f, 5000.0f);  //z_far = 500.0f by default
...
void gui_start_basic_structure(gui_structure& gui, scene_structure& scene)
{
    ...
    ImGui::Begin("GUI",nullptr,ImGuiWindowFlags_AlwaysAutoResize); // used to have NULL
